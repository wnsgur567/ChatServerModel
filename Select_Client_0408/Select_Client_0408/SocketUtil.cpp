...
int SocketUtil::Select(
	const vector< TCPSocketPtr >* inReadVec,	//	recv를 감시해야할 socket들
	vector< TCPSocketPtr >* outReadVec,			//	select 가 끝난 후 socket들
	const vector< TCPSocketPtr >* inWriteVec,	//	send를 감시해야할 socket들
	vector< TCPSocketPtr >* outWriteVec			//	select 가 끝난 후 socket들
...
)
{
	...
	fd_set* writePtr = FillSetFromVector(read_set, inWriteVec);
	//fd_set* exceptPtr = FillSetFromVector(read_set, inExceptSet);
...
fd_set* SocketUtil::FillSetFromVector(
	...
	const vector<TCPSocketPtr>* inSockets	// set에 넣을 socket vec
)
{
	if (inSockets)
	{	// not null
